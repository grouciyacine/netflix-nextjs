...
function Update({ setUpdate ,id}: Props) {
    ...
    const addProfile = useCallback(async () => {
        const response = await axios.patch('/api/addProfile', { img: name, title: title,id:id })
    ...
    return (
        <div className='absolute w-1/2 border-[1px] border-zinc-600 p-3 h-2/3 top-11 bg-black bg-opacity-80'>
            <TfiClose className='text-lg text-white absolute right-0 m-2 cursor-pointer hover:text-zinc-300' onClick={() => setUpdate(false)} />
            ...
            <button onClick={addProfile} className='p-2 bg-red-600 m-4 hover:bg-red-800 transition-all duration-200 text-white right-11 rounded-md justify-center items-center text-center'>Update</button>
        </div>
    )
}
